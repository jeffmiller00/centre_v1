...
Plugin URI: http://www.sohailabid.com
...
define('SOHAIL_EMBED_FACEBOOK_URL', 'http://localhost/TheCentre');

function sohail_embed_facebook_head() {

	echo '<script type="text/javascript" src="'.SOHAIL_EMBED_FACEBOOK_URL.'/slimbox2/js/slimbox2.js"></script>' . "\n";
	echo '<link rel="stylesheet" href="'.SOHAIL_EMBED_FACEBOOK_URL.'/slimbox2/css/slimbox2.css" type="text/css" media="screen" />' . "\n";
	echo '<link rel="stylesheet" href="gallery.css" type="text/css" media="screen" />' . "\n";

	echo '<script type="text/javascript">
<!--
    function sohail_expand_content(id) {
    	//jQuery("#"+id).show();
    	//jQuery("#sohailmorelink"+id).hide();
...
function sohail_do_embed($url) {
	...
	if(!$query_vars) {
		...
		if(strpos($raw_query_string[0], 'album.php') !== false)
			...
			if (strpos($album->link, $query_vars['aid'])) {
				...
				if(!isset($fb_photos->data)) {
					...
					foreach($fb_photos->data as $photo) {
						$return .= "<a rel='lightbox-fb' href='{$photo->source}' title='{$photo->name}'><img src='{$photo->picture}' class='sohailfbthumb' /></a>";
					}
				}
				$return .= "</div>\n";
...
function sohail_do_embed_photo($query_vars, $url) {
	...
	if(!isset($fb_photo->source)) {
		...
		$return .= "<div class='sohailfbboxbody'>\n";
		$return .= "<a rel='lightbox-fb' href='{$fb_photo->source}' title='{$fb_photo->name}'><img src='". str_replace('_s.jpg', '_a.jpg', $fb_photo->source) ."' style='max-width:100%' /></a>";
		$return .= "</div>\n";
		$return .= "</div>\n";
...
function sohail_do_embed_event($query_vars, $url) {
	...
	if(!isset($fb_event->name)) {
		...
		$start_time = sohail_iso8601($fb_event->start_time)-28800;
		$end_time = sohail_iso8601($fb_event->end_time)-28800;
